...
from api_utils.json_response import JsonResponse as Response
...
from api_utils.custom_pagination import UserPagination
...
from api_utils.viewset_base import CustomViewBase

# class ChargeOrderViewSet(ModelViewSet):
class ChargeOrderViewSet(CustomViewBase):
...
from .serializer import ChargeSummarySerializer
class ChargeSummaryViewSet(CustomViewBase):
    queryset = ChargeSummary.objects.all()
    serializer_class = ChargeSummarySerializer
    pagination_class = UserPagination
    permission_classes = [permissions.IsAuthenticated]


from django.http import Http404
class ChargeSumOfPatient(APIView):
    from api_utils.json_response import JsonResponse as Response
    """
    ...
    def get_object(self, pk):
        p = Person.objects.get(pk=pk)
        try:
            return ChargeSummary.objects.get(person=p)
        except ChargeSummary.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        snippet = self.get_object(pk)
        serializer = ChargeSummarySerializer(snippet)
        return Response(data=serializer.data, code=20000, msg="success", status=status.HTTP_200_OK)

    def put(self, request, pk, format=None):
        snippet = self.get_object(pk)
        serializer = ChargeSummarySerializer(snippet, data=request.data)
...
class ChargeOfPatient(APIView):
    from api_utils.json_response import JsonResponse as Response
    from api_utils.CustomAuthToken import CsrfExemptSessionAuthentication
    ...
    def get_object(self, pk):
        p = Person.objects.get(pk=pk)
        try:
            return ChargeOrder.objects.filter(person=p)
    ...
    def get(self, request, pk, format=None):
        ...
        serializer = ChargeOrderSerializer(order, context=serializer_context, many=True)
        return Response(data=serializer.data, code=20000, msg="success",
                        status=status.HTTP_200_OK)

    def post(self, request, pk, format=None):
        serializer_context = { 'request': request,}

        serializer = ChargeOrderSerializer(data=request.data, context=serializer_context)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, code=20000, msg="success", status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
